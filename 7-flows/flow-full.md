...
**Harness 适配**：H — 仅完整 Harness 级别使用此超级流程编排；M 级别走子流程的标准 5 阶段管道即可。
...
### 五阶段模型
flow-full 将项目生命周期划分为 **5个阶段（Phase）**，每个阶段路由到对应的 **子流程（Sub-flow）**。

```
Phase 1: 需求分析
  │
  ├──→ [子流程] flow-iteration (需求迭代流程)
  │      编写需求文档 → 评审需求 → 确认需求基线
  │
  ├── [里程碑 M1: 需求基线确定]
  │   交付物: 需求文档、用户故事、验收标准
...
Phase 3: 开发实现
  │
  ├──→ [子流程] flow-audit (代码审计流程)
  │      开发规范 → 代码实现 → 代码审计 → 安全审查
  │
  ├──→ flow-audit 可并行拆分为多个子任务
  │      每个模块独立审计
  │
  ├── [里程碑 M3: 核心功能完成]
  │   交付物: 代码库、审计报告、模块测试结果
...
Phase 4: 测试验证
  │
  ├──→ [子流程] flow-testing (测试优化流程)
  │      测试计划 → 用例编写 → 自动/手动测试 → 覆盖率分析
  │
  ├── [里程碑 M4: 测试通过]
  │   交付物: 测试报告、覆盖率报告、缺陷列表
  ▼
...
Phase 5: 文档完善   ← 文档与发布
  │
  ├──→ [子流程] flow-docs (文档完善流程)
  │      架构文档 → API文档 → 用户手册 → 部署文档
  │
  ├── [里程碑 M5: 交付完成]
  │   交付物: 完整文档集、部署脚本、验收报告
  ▼
...
4️⃣ 保存当前状态 → /reqplan context snapshot
"""
```
...
**版本**: 3.3
**更新时间**: 2026-05-14
**引用**: 7-flows/flow-iteration.md, 7-flows/flow-design-review.md, 7-flows/flow-audit.md, 7-flows/flow-testing.md, 7-flows/flow-docs.md, 7-flows/flow-refactor.md, 3-core/core-actions.md, 3-core/core-context-tracker.md, 3-core/core-state-management.md
