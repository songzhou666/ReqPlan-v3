...
### 执行内容
1. 检查项目根目录是否已有 AGENTS.md
   - 有 → 跳过，读取现有内容
   - 无 → 创建 AGENTS.md（按 template-agents 结构），填入项目名称、技术栈、验证命令
2. 检查 `.agent/plans/` 目录是否存在
   - 无 → 创建 `.agent/plans/` 目录
...
### 完成条件
AGENTS.md、.agent/plans/、docs/harness/history.yaml 已就位。
然后询问用户下一步任务。
...
| 情况 | 处理方式 |
|------|---------|
| AGENTS.md 不存在 | 按 template-agents 创建，填入已知信息 |
| .agent/ 目录不存在 | 创建 .agent/plans/ |
...
### Step 1 理解需求
引导用户澄清功能点、角色、数据和约束。
**产出**：按 template-req 结构输出的需求摘要
**完成条件**：需求摘要已与用户确认
**回写**：将需求摘要保存到 .agent/requirements/（确保目录存在）
**门控**：→ 用户确认后进入 Step 2

### Step 2 设计方案
根据需求确定技术方案：架构、模块划分、接口设计、数据模型。
**产出**：按 template-design 结构输出的设计方案
**完成条件**：技术方案已与用户确认
**门控**：→ 方案确认后进入 Step 3

### Step 3 制定计划
将方案拆解为可执行的任务列表，每项任务标注涉及文件和验证方式。
**产出**：按 template-plan 结构输出开发计划，创建 .agent/plans/{date}-{module}.md（确保目录存在）
**完成条件**：计划已创建，任务列表初始状态全部为 ⏳ TODO
**门控**：→ 计划就绪后进入 Step 4

### Step 4 执行实施
按计划逐个任务实施，每完成一个更新计划中的状态列。
**状态更新规则**：
- 开始实施某任务 → ⏳ TODO 改为 🔄 WIP
- 完成实施 + 验证 → 🔄 WIP 改为 ✅ DONE
...
**产出**：验证结果摘要，记录到 plan 文件末尾
**完成条件**：五层全部通过，或已记录已知例外
**门控**：→ 验证通过后进入 Step 6

### Step 6 链路回写
- 追加记录到 docs/harness/history.yaml（追加，不要覆盖已有记录）
...
- 复杂修复：输出 plan 模板到 .agent/plans/{date}-{fix}.md（确保目录存在），进入 Step 4
**完成条件**：方案已与用户确认
**门控**：→ 方案确认后进入 Step 4

### Step 4 实施修复
编码实现 + 测试验证。
复杂修复时按 plan 任务列表逐步执行，更新状态。
**完成条件**：修复编码完成 + 自测通过
**门控**：→ 修复完成进入 Step 5
...
### Step 6 链路回写
追加记录到 docs/harness/history.yaml，诊断记录保存到
docs/harness/fixes/{date}-{issue}.md（确保目录存在）。
**完成条件**：链路已更新
**门控**：→ 流程结束
