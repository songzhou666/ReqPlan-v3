...
四大场景共享同一个工作管道：**理解 → 设计 → 实施 → 验证**，区别在于起点不同。

```
开发：起点是"需求" → 走完整管道
分析：起点是"目标" → 侧重理解与设计阶段
修复：起点是"问题" → 侧重诊断与验证阶段
审查：起点是"检查" → 侧重分析整体质量与一致性（元任务：审查 Skill 自身）
```

> 本 chunk 定义的 4 场景 Step 流程与主 SKILL.md 的 7 阶段状态机（START→ANALYZE→...→JUDGE）**互补而非替代**：
...
Layer 1 静态检查 → Layer 2 单元测试 → Layer 3 构建集成 → Layer 4 异常处理 → Layer 5 流程合规
**对应状态机阶段**：VERIFY
...
## 流程4：自绑定审查（审查 / 修复 Skill 自身）

> **🪞 自绑定**：本流程专用于审查、检查、分析或修复 reqplan-v3 Skill 自身。
> 它使用与流程1相同的 7 阶段状态机，只是各阶段的产出物从"项目代码"变为"Skill 文件本身"。

对应 SKILL.md 7 阶段主流程（**无跳过**，完整7阶段）。任何以"元任务"为由跳过状态机的行为均违反自绑定条款。

### Step 1 分析Skill结构（ANALYZE）
**对应状态机阶段**：ANALYZE  
**产出**：`.agent/harness/_analysis.md`  
**分析维度**：
1. 文件结构完整性（所有引用文件是否存在）
2. 流程闭环性（START→...→DONE 是否完整）
3. 状态机合理性（节点、流转、条件）
4. 文件一致性（版本号、交叉引用、模板）
5. Skill 通用性（是否过耦合、硬编码）
6. 自约束有效性（AI是否可能绕过规则）
**完成条件**：所有分析维度已完成
**门控**：→ 分析完成进入 CONFIRM
...
**完成条件**：用户确认或提出修改意见
- ✅ 确认 → 进入 DESIGN
- ✏️ 修改 → 记录修改内容，返回 ANALYZE

### Step 3 设计修复方案（DESIGN）
**对应状态机阶段**：DESIGN  
**产出**：`.agent/harness/_design.md`  
**设计内容**：
1. 针对发现的问题设计修复方案
2. 列出涉及的文件和修改内容
3. 明确每个修改的验证方式
...
### Step 4 实施修复（IMPLEMENT）
**对应状态机阶段**：IMPLEMENT  
**产出**：`.agent/harness/_implementation.md`  
**执行方式**：按设计任务列表逐一实施，写入文件
**完成条件**：所有修复任务已完成
**门控**：→ 实施完成进入 VERIFY
...
**产出**：`.agent/harness/_verification.md`  
**验证内容**：
1. 文件校验：列出所有修改的文件并确认写入成功
2. 一致性检查：版本号、交叉引用、标题、目录结构
3. 流程重演：在思维中重演一次完整状态机
**完成条件**：所有验证项通过
**门控**：→ 验证通过进入 JUDGE
...
**判定标准**：
- ✅ PASS：全部修复通过，文件一致
- 🔧 缺陷等级为 P0/P1 → 返回 DESIGN 修复
- ❌ FAILED：严重问题无法修复 → 输出终止报告
**产出**：`.agent/harness/_verification.md` + `.agent/harness/_quality_audit_judge.md`  
**完成条件**：判定结果确定，接力棒更新
**门控**：→ 流程结束